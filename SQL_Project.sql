...
-- 3. Data Insertion (Truncated for brevity, use full project_data.sql for execution)
INSERT INTO hospitals VALUES (1, 'Green Valley Medical Center', '123 Elm St, Springfield', '555-123-4567'),
...
SELECT * FROM prescriptions WHERE patient_id = 5;
...
-- Check total records in each major table
...
JOIN appointments a ON p.patient_id = a.patient_id
JOIN doctors d ON a.doctor_id = d.doctor_id
GROUP BY p.patient_id, p.name, p.address, p.phone
HAVING COUNT(DISTINCT d.hospital_id) > 1;

SELECT p.name, MAX(a.appointment_date) AS latest_appointment
FROM patients p
JOIN appointments a ON p.patient_id = a.patient_id
GROUP BY p.patient_id, p.name;

SELECT m.name, COUNT(p.prescription_id) AS prescription_count
FROM medications m
...
SELECT h.name, COUNT(d.doctor_id) AS doctor_count
FROM hospitals h
LEFT JOIN doctors d ON h.hospital_id = d.hospital_id
GROUP BY h.hospital_id, h.name
HAVING doctor_count = (
    SELECT MIN(doc_count)
    FROM (SELECT COUNT(doctor_id) AS doc_count FROM doctors GROUP BY hospital_id) AS counts
);

WITH DeptCapacities AS (
    SELECT 
...
SELECT hospital_name, department_name, total_capacity
FROM DeptCapacities
...
-- Top Hospitals with the Most Specialists
-- Per Hospital
SELECT 
    h.name AS hospital_name, 
...
WHERE d.specialty = 'Cardiology' -- You can change this to any specialty
GROUP BY h.hospital_id, h.name
ORDER BY specialist_count DESC;

-- Average Room Capacity per Department and Hospital
-- Per Department
SELECT 
    d.name AS department_name, 
